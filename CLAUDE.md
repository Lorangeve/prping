...
- **模块地图**（逐文件细节看代码注释与 docs/claude-rules/，此处只列职责）：
  ...
  - `web/` — Web 编辑器服务器（`prping web`，单端口 HTTP+WS，smol）：`http.rs` 极简 GET/HEAD、`ws.rs` 信封协议（lsp 透传/analyze/list/read/tree/save/delete/workspace/run/run_stop）+ FrameDecoder 防御、`run.rs` 执行桥（run 信封 spawn 自身二进制 `packet` 子命令：输出/退出流式回传 run_out/run_exit；工作区内 .pkt/.pktl 限定 + 子进程 cwd=工作区根 + --out validate_rel 防逃逸 + 同连接多标签并行运行（Run 面板任务列表 = 任务管理：点行切换控制台/■ 停止/✕ 移除；■ Stop all 悬浮列全连接运行清单——跨标签可见、行内 ■ 可单停）+ 全服并发上限 8 + 输出封顶 + run_stop 按 id 停单个/缺省全停 + 断开/会话收尾即全部 kill + stdin 生命线（PRPING_WEB_RUN=1 子进程监视 stdin EOF，服务端死亡无孤儿））、`workspace.rs` 可写工作区（默认 `examples/`，发现顺序同 `UI/`；`workspace` 信封打开自定义目录、`browse` 列目录供 web 文件夹选择对话框（打开文件夹唯一入口）；写操作限根内：路径逐段校验 + canonicalize 根内断言 + 4MB 上限；读/删（文件直接删、目录递归删）/改名/建目录接受任意文件（.pkt/.pktl 由 tree 的 pkt 标记区分；二进制文件读取明确报错），`rename`/`mkdir` 信封做重命名/移动/建目录）、`pipe.rs` 异步↔阻塞桥（零改动复用 `run_lsp_on`）、`assets.rs` 静态资源双模式——**默认运行期读 `UI/` 目录（二进制所在目录优先，其次启动目录）**；`--features web-embed` 时 rust-embed（debug 直读 dist / release 编译期内嵌）；键名逐段校验（空段/点段/反斜杠/冒号拒绝）；模块整体由 `web` feature 门控，默认不编译
...
- **`web`**（`--features web`，默认关）：门控 `prping web` **整个子命令**（core 的 `serve_web`/`WebConfig` 与 CLI 的 web 子命令）。关闭时 web 模块不编译、CLI 无 web 子命令（产物体积更小）。开启用 `--features prping/web`（workspace 需包名限定，避免与 prping-cli 同名 feature 歧义）；**justfile 的 build/check/test 配方统一启用**，产物默认具备完整功能。web 模块仅用已有 crate（smol/termcolor/async-tungstenite/open/futures-util/mime_guess/serde_json），无新增依赖。
- **`web-embed`**（`--features web-embed`，默认关；**隐含启用 `web`**）：rust-embed 编译期内嵌前端进二进制（单文件分发）。默认不内嵌：运行期读二进制同目录 `UI/`。前端构建两条路径：① web-embed 下由 `crates/prping-core/build.rs` 执行（rust-embed 读盘在 core 编译期，构建逻辑必须先行）；② 默认模式下由 justfile 的 `web-dist` 保障配方按需自动构建（dist 缺失或前端源码更新时，`just build`/`just build-release` 的依赖；产物新鲜零开销）。两条路径同为 bun 优先、npm 降级、`PRPING_SKIP_WEB_BUILD=1` 跳过、无 node 仅警告。
...
prping packet [OPTIONS] FILE.pkt|.pktl [HOST:PORT]  构建发送/配方执行（--raw/--wait[SECS]/--fuzz/--out；裸 --wait = 纯持续监听（sniffer 匹配+报告），--wait --raw = 链路层纯监听；回应包一律由 .pktl 配方编排）
...
4. **Web 编辑器** — SolidJS SPA（CodeMirror 6）+ engine LSP（WS 信封桥接 `run_lsp_on`：诊断/补全/悬停/定义跳转）+ **执行**（Run 页签/顶栏 ▶ 运行工作区 .pkt/.pktl：`run`/`run_stop` 信封 → 服务端 spawn 自身 `packet` 子命令（零改动复用 CLI 全语义，ADR #8），params/count/wait/listen/fuzz/raw/iface/json（默认开）选项快照持久化（**无 target**：地址由包字段 params 承载、引擎从包内推导 socket 目标；显式 HOST:PORT 只留 CLI 位置参数；.pktl 的参数输入行 = 服务端聚合步骤 .pkt 的 params("名",默认) 声明 + 步骤 params: 覆盖标记）、未保存先落盘再执行、`run_out`/`run_exit` 流式聚合到执行控制台——json 模式按 JSONL 解析渲染（✓/✗ 包行+层栈/字节/目标/错误、步骤行、汇总行，悬停看原始 JSON），非 JSON 行原样回退，listen 与 json 互斥（CLI 校验）；参数输入与 Layers 页签共用同一份 `runValues`）+ **文档标签页**（每打开一个文件一个编辑器 tab，多文件并行编辑：切走落账/切回还原未保存内容，✕ 或 Alt+W 关闭，脏文件先写 IndexedDB 草稿防丢、重开走恢复提示；右栏数据随标签隔离——运行任务卡按 fileKey 归属、运行参数输入每标签独立，关闭文件标签停止其运行中任务）+ 点击跳转与导航历史（`.pkt` 内 Ctrl/Cmd+单击标识符跳定义（Alt = 多光标/块选）——跨文件经 packet-dsl `Module::definition_of` 解析 import/转出口/库 prelude 到目标 `.pkt` 源文件；import 模块名与 `.pktl` 步骤文件名同按修饰键单击，无修饰键单击永不跳转；高亮集合 = 精确可跳集合（本地符号 + import 名 + 非内置调用名，内置名单经 `config.json` builtins 下发），按住修饰键统一点亮；顶栏 ←/→ 与 Alt+←/→ 返回前进，落点带行、离开回填）+ 实时层栈/HEX 预览（`analyze_text_json` 与 CLI `--json` 同构）+ 左栏文件管理（工作区 = 启动目录 `examples/` 或页面打开的自定义目录，可编辑/保存 Ctrl+S/新建/删除（目录递归，确认文案说明后果）/改名/移动 + 行悬停按钮与右键菜单（文件：打开/改名/删除；目录：改名/删除；空白区：新建/刷新）+ 目录树折叠（默认全收起）；eng_lib 只读浏览；📂 = web 文件夹选择对话框（browse 导航，默认当前工作区根；唯一入口，原生选择框/webkitdirectory 导入因重复与路径限制已移除）；前端 IndexedDB 持久化项目状态——工作区根/上次文件/未保存草稿/面板，生成物不落库；打开真实文件即以真实 `file://` URI 走 LSP/analyze，import 解析到文件所在目录）；`--open` 开浏览器；前端**默认不内嵌**（运行期读**二进制同目录** `UI/`：`just build`/`just build-release` 经 `web-dist` 按需自动构建前端、经 `just dist` 同步 `target/<profile>/{UI,lib,examples}` 完整可运行布局；源码树不留生成物），`--features web-embed` 才内嵌（单文件分发）；`web` 子命令本体由 `--features web` 门控，justfile 的 build/check/test 配方统一启用
