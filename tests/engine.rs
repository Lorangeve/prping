...
fn completion_offers_keywords_and_builtins() {
    ...
    assert!(
        tcp["detail"].as_str().unwrap().contains("func tcp("),
        "tcp detail: {:?}",
        tcp["detail"]
    );
}
...
fn hover_shows_lib_func_docs() {
    ...
    // udp 是 eng_lib 库函数（隐式可见）：显示 func 签名与参数
    assert!(md.contains("### `func udp("), "{md}");
...
fn analyze_file_with_libs_resolves_imports() {
    ...
    // 无 libs → 找不到模块
    let out = prping::analyze_file(&main, &[], &[]).unwrap_err();
    assert!(out.to_string().contains("找不到模块"), "{out}");

    // 带 libs → 成功
    prping::analyze_file(&main, &[], std::slice::from_ref(&lib)).expect("带 libs 应成功");
