...
| 文件 | 角色 | 配方步骤 |
| --- | --- | --- |
| `server.pktl` | 服务端（按端口分派） | **显式多组** listen+reply：第 1 组 `wait: -1` **UDP 数据报监听** :5533 匹配 DNS 查询（flags=0x0100）→ extract id/对端地址写 global → 触发发包 A 记录应答；第 2 组 `wait: -1` + `raw: true` **链路层监听** 匹配 RFB 横幅（跨层 AND：udp dport=5900 + raw 前缀）→ extract 帧内对端 → 回横幅。两组对应 client 两步；要服务更多次就多写几组 |
