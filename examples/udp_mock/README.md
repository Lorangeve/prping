...
| 文件 | 角色 | 配方步骤 |
| --- | --- | --- |
| `server.pktl` | 服务端（按端口分派） | **显式多组** listen+reply：第 1 组 `wait:` 无值 **UDP 数据报监听** :5533 匹配 DNS 查询（flags=0x0100）→ extract id/对端地址写 global → 触发发包 A 记录应答；第 2 组 `wait:` 无值 + `raw: true` **链路层监听** 匹配 RFB 横幅（跨层 AND：udp dport=5900 + raw 前缀）→ extract 帧内对端 → 回横幅。两组对应 client 两步；要服务更多次就多写几组 |
...
```bash
# 终端 1 —— 服务端配方（需 root：第 2 组是链路层监听，抓帧要 cap_net_raw）
...
example.com → 93.184.216.34），步骤 2 `sent 12 B, received 12 B`；服务端两组
各命中一次并回包。跨机实验给客户端加 `-p ip=<对端 IP>`（回包按 socket 对端
地址原路返回，天然支持）。
...
- 服务端配方的每个 listen 步骤按**自己 .pkt 的最外层 udp dport**（或链路层
  规则里的 dport 条件）绑定/过滤各自的端口——两组独立监听 :5533 与 :5900；
...
5533 全程免 root，把 root 的需求留给真正绕不开的第 2 组（见下）。代价是与
真实 DNS 端口不同——教学演示取舍，真实解析器仍固定用 :53。

## 为什么第 2 组（VNC）必须走链路层监听
...
配方形态与第 1 组同构，教学重点是横幅格式与端口分派，不是真实时序。横幅之后
...
- **链路层监听**：listen 步骤 `raw: true` → 抓完整帧（需 root）；帧内
  ipv4/udp/raw 字段齐全；对端从帧内取；
- **触发发包**：listen 命中 → extract 写 global → 后续步骤用
...
服务端配方每组监听是**串行**的：命中第 1 组 → 关闭 :5533 socket → 发应答 →
打开第 2 组 :5900 监听（链路层抓包初始化更慢）。回环上 client 全程 <1ms 就
跑完，不加 delay 会把横幅发在 :5900 监听就绪之前——服务端永远等不到（表现为
第 2 组 2s 超时）。这与 `examples/icmp_mock/`「为什么 seq 是 1001/1002」
一节是同一条回环经验。
...
- 8 个文件 `prping engine` 全部通过（reply 两个文件引用配方运行时才有的
  ...
  `-g tid=... ` 注入即过——语法解析均成功）；
- DNS 组免 root 端到端闭环：服务端命中查询 → extract tid/cip/cport → 回
  A 记录应答；客户端 `✓ reply matched: id=4660 flags=33152`，应答反解出
  rdata=5db8d822（93.184.216.34）；
- 第 2 组链路层规则经 `--pcap` 逐帧反解验证：横幅完整帧反解出
