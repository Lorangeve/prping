...
- MSVC 构建静态链接 CRT 与 C++ 运行库（`.cargo/config.toml` 配 `crt-static`）：产物不依赖 `vcruntime140.dll`/`msvcp140.dll`/`ucrtbase.dll`，目标机器无需 VC++ Redistributable；实测 `x86_64-win7-windows-msvc` 产物仅依赖 `ADVAPI32`/`KERNEL32`/`ntdll` 三个 Win7 自带系统库
