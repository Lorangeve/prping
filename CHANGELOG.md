...
- **配方字段空值一律非法（`.pktl` 语法收紧）**：字段「要么不写、要么写值」——空值易误读
  语义。`wait:` 空值（原 = 持续监听）与 `- loop:` 空值（原 = 无限循环）改为**解析报错**，
  两语义改用显式负数 `wait: -1` / `loop: -1`（与 CLI 负数 `--wait` 同语义，报错文案带
  迁移提示）；`extract:` 内联值（原被静默忽略）与 `params:` 空值（原静默无操作）同样
  报错；`init:` / `from:` / `as:` / `on_error:` / `delay:` / `count:` 空值的报错统一为
  「不能为空 + 期望形态」。全部示例迁移 `wait: -1`，LSP `wait:`/`loop:` 值位补全 `-1`
