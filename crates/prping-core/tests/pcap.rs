...
use prping_core::{LinkType, PkgOptions, SendMode, read_pcap, write_pcap};
...
fn linktype_inference() {
    ...
    let pkt = packet_dsl::resolve(&m)
        ...
        .next()
        .unwrap();
    assert_eq!(prping_core::linktype_of(&pkt.layers), LinkType::Ethernet);
    ...
    let pkt2 = packet_dsl::resolve(&m2)
        ...
        .next()
        .unwrap();
    assert_eq!(prping_core::linktype_of(&pkt2.layers), LinkType::Raw);
}

/// --pkt --out：构建 + 存档（不发目标也能写 pcap）。
#[test]
...
fn send_packets2(
    ...
    prping_core::send_packets(file, &o)
}
