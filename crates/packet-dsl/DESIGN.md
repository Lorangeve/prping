...
### 6.6 静态形状检查（求值前，`check.rs`）

> §6.1–6.5 是**求值期**的权威转换规则。`check.rs` 把其中**语法可判定**的子集提前到
> 编辑器：解析成功后对 def/流水线/函数体与参数默认值/值函数体/proto 字段默认值/
> sniffer 值表达式做**零误报**检查，文案与对应 coercer 报错对齐
> （`int_of`/`encode_int_bytes`/`bytes_of`/`tpl`）——「编辑器看到的就是运行时会发生的」。

- **必报**（求值期必然失败的形态）：字符串字面量进数值位（u8/be16/.../shl/shr/
  bnot/rand_bytes/pad/dns_name 等的 `int_of` 路径）；整数字面量越界（0..=2^n−1，
  8 字节 i64 封顶）；同宽直通宽度不符与字节列表元素越界（宽度可静态判定时——
  合法 `hex("...")` 解析期已转字节列表，残留 `hex` 调用标记即非法输入）；
  `+` 非整数操作数；shl/shr 移位量 ∉ 0..64；`div` 字面量除零；rand_bytes/pad
  超 0..65535；count/cksum/md5/sha1/sha256/dns/global/tpl/ip4/ip6/mac 的字面量
  参数类型错；proto 字段默认值按字段类型（含 bits 判别位、`bytes=宽度`）。
- **跳过**（零误报优先）：`params(...)`（形状运行时才定）、裸 ident（参数/前序
  字段引用）、未知/库/用户值函数的调用（体不跨模块展开；`ip4`/`ip6`/`mac` 例外
  ——标准库地址值函数按 tpl 宽度表检查）、sniffer 字面量（常量比较语义）。
- **宿主接入**：LSP `analyze` 在解析成功后运行（多诊断按位置排序去重；解析失败
  仍报单条解析错误），与打词豁免、悬停/补全共用同一解析管线；CLI 求值路径不变
  （本检查只提前报错时机，不改语义）。
