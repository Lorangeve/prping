...
impl DefaultSerializer {
    ...
    fn serialize_icmp(&self, f: &IcmpFields, payload: &[u8]) -> Result<Vec<u8>, SerializeError> {
        if let Some(raw) = &f.raw {
            ...
            // `payload` 字段（dissect 反解的 body）追加在头字节之后、内层之前——
            // dissect 的 raw 只含 8B 头，body 在 f.payload；否则 icmp 在最内层时 body 会丢失。
            let mut hdr = raw.clone();
            if hdr.len() >= 4 {
                hdr[2] = 0;
                hdr[3] = 0;
                let mut seg = hdr.clone();
                if let Some(body) = &f.payload {
...
/// BE 16 位字求和（奇数字节补高 8 位；**不折叠**）——互联网校验和的求和部分，
...
/// 反码进位折叠至 16 位（无界循环，任意大小收敛）——自动校验和与 DSL `cksum`
/// 原语共用。
pub(crate) fn fold_sum(mut sum: u64) -> u64 {
    while sum >> 16 != 0 {
...
pub fn checksum(data: &[u8]) -> u16 {
    !(fold_sum(word_sum(data, 0)) as u16)
}
