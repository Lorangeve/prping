...
full = use(h) |> tcp(sport=40000, dport=80, seq=1, flags="syn,ack")
...
fn http_auto_content_length() {
    let src = "h = http(method=\"POST\", path=\"/login\", headers=[\"Content-Length: 2\"], body=\"{}\")\nuse(h) |> tcp(dport=80) |> ipv4() |> eth()\n";
    let bytes = build_bytes(src);
    let pkt = &bytes[0];
