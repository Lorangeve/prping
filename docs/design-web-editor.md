...
- **run 生命周期事件不占请求 id**：`run` 信封的 `id` 仅用于启动 ack（成功 = 活跃，
  ...
  关联，与请求-应答配对机制解耦。**同连接可并行多个运行**（Run 面板任务列表
  查看/停止/切换，控制台跟随选中的任务），全服并发由
...
└── src/
    ├── index.tsx      入口 render
    ├── App.tsx        布局（顶栏——含返回/前进导航钮/左栏文件管理/文档标签条/编辑器/右面板）
    │                 + 数据流编排 + 打开/保存 + 跳转目标映射（URI → 工作区/库文件）
    │                 + 导航历史栈（落点带行、离开回填，§5.4）
    │                 + 文档标签页（每打开一个文件一个 tab；切走落账/切回还原未保存
    │                   内容；✕/Alt+W 关闭，脏文件先写 IndexedDB 草稿防丢）
...
- **文档标签页**：每打开一个文件一个 tab（编辑器列顶部标签条），多文件并行编辑——
  切走时把编辑器实时内容落账进标签（未保存修改驻内存），切回即还原（不重读磁盘、
  不丢修改）；已开文件的再次打开 = 激活既有标签；✕ 或 Alt+W 关闭（脏文件先写
  IndexedDB 草稿，重开时走恢复提示）；关闭当前标签激活相邻（右优先），全部关闭
  回示例文档空态；重连后旧会话标签清空（内容已落盘/在草稿里，不丢数据）；
  **右栏数据随标签隔离**：运行任务（fileKey 归属）与运行参数输入
  （runValues/runParams）都是每标签独立——Run 面板只显示当前文件的任务，
  任务列表有任务即常驻（点行切控制台/■ 停止/✕ 移除），文档标签上的 ● 指示
  其它文件还有任务在跑；关闭文件标签会停止该文件的运行中任务；
