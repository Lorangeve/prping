...
> （WSAPoll / ICMP.DLL / raw 收包无外层头 / trace --tcp 限制）见
...
### TCP SYN 变体（`trace --tcp HOST:PORT`，`cfg.trace_tcp`）

对标 tcptraceroute/tracetcp。`trace_tcp`（unix 专用，Windows 报错）——raw TCP socket
（`IPPROTO_TCP`）发 SYN（内核构 IP 头）、伪头部校验和用 `engine::pkg::local_ip_for` 的
本地源 IP 并 bind 到该地址；每探测独立源端口 0x4000..=0x5FFF（防 IPv6 无头缓冲被
version-nibble 误判为带头）；raw ICMP（Time Exceeded，按内嵌 TCP (sport,dport) 匹配）
+ raw TCP（SYN-ACK/RST，源=目标 IP 且端口匹配；`match_tcp_reply` 先无头解析兜底再按
带头）双 socket `libc::poll` 等待，两个 socket 先于发送打开；SYN-ACK 与 RST 都算到达。
中间路由回 Time Exceeded，目标回 SYN-ACK（端口开）/RST（端口关）即到达（ICMP 被过滤时
仍可用）。
...
ICMPv6 type 1 code 4）到达，按内嵌 UDP 头 (sport, dport) 匹配归属）；`--tcp` 与
`--udp` 互斥（`TraceProtoConflict`）。
