//! Windows Npcap raw 发送兼容层（`--pkg --raw` 的 Windows 后端）。
//!
...
#[cfg(windows)]
use crate::pkg::{Reply, SendOutcome};
#[cfg(windows)]
...
/// `--pkg --raw` 的 Windows 发送入口（pkg.rs::send_raw_bytes 在 windows 下委托）。
///
...
pub(crate) fn send_raw_full(
    ...
    iface: Option<&str>,
    wait: Option<f64>,
    sniffer: Option<&crate::pkg::SnifferMatcher>,
    ...
    let (proto, frame) = match pkt.layers.last() {
        ...
        Some(Layer::Ipv4(_)) => ("IP4", wrap_ip4(bytes, target)?),
        Some(Layer::Ipv6(_)) => ("IP6", wrap_ip6(bytes, target)?),
    ...
    let reply = match wait {
        Some(secs) => wait_reply(&mut cap, secs, pkt, sniffer, sent_report)?,
        None => None,
    };
    Ok(SendOutcome {
        proto,
...
#[cfg(windows)]
fn select_device_name(target: &SocketAddr, iface: Option<&str>) -> anyhow::Result<String> {
    let devs = list_devices()?;
    let idx = pick_device(&devs, target.ip().is_loopback(), iface).ok_or_else(|| {
    ...
    let dst_mac = match dst {
        ...
        None => {
            let _ = crate::output::writeln_orange(
                ...
                format!(
                    "  note: 无法解析 {next_hop} 的 MAC（ARP 缓存未命中）——用广播地址发送，回包可能无法到达"
                ),
            );
            [0xff; 6]
        }
    ...
    let src_mac = match local_mac(if_index) {
        ...
        Err(e) => {
            let _ = crate::output::writeln_orange(
                &mut crate::output::stderr(),
                format!(
                    "  note: 获取接口 {if_index} 的 MAC 失败（{e}）——用全零源 MAC，回包可能无法路由回来"
                ),
            );
            [0u8; 6]
        }
...
fn wait_reply(
    ...
    if sniffer.is_none() && crate::pkg::icmp_echo_ids(pkt).is_none() {
        return Ok(None);
    }
    let t0 = Instant::now();
        ...
        match cap.next_packet() {
            Ok(p) => {
                ...
                if let Some((bytes, matched)) =
                    crate::pkg::match_reply(p.data, pkt, sniffer, sent_report)?
                {
